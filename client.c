...
int main(int argc, char *argv[])
{
    ...
    while ((ch = getch()) != KEY_F(1))
    {
        ...
        case 127:
            ...
            {
                int y1, x1, y2, x2;
                selection_get_normalized(&y1, &x1, &y2, &x2);
...
static void apply_server_message(NetMessage *msg)
{
    ...
    case S2C_INSERT:
        ...
        if (y >= MAX_LINES || x > local_doc.line_lengths[y] || x >= MAX_LINE_LEN)
            return;
        if (y >= local_doc.num_lines)
            local_doc.num_lines = y + 1;
        ...
        if (msg->user_id == my_user_id)
            user_cursor.x++;
        else if (y == user_cursor.y && x < user_cursor.x)
            user_cursor.x++;
    ...
    case S2C_DELETE:
        ...
        if (y >= local_doc.num_lines || x <= 0 || x > local_doc.line_lengths[y])
            return;
    ...
    case S2C_DELETE_FORWARD:
        ...
        if (msg->user_id != my_user_id && y == user_cursor.y && x < user_cursor.x)
            user_cursor.x--;
    ...
    case S2C_NEWLINE:
    {
        ...
        if (y >= local_doc.num_lines || x > local_doc.line_lengths[y] || local_doc.num_lines >= MAX_LINES)
            return;
    ...
    case S2C_FORMAT_RANGE:
    {
        ...
        for (int yy = r.y1; yy <= r.y2; ++yy)
        {
            ...
            if (sx < 0)
                sx = 0;
            if (ex > local_doc.line_lengths[yy])
                ex = local_doc.line_lengths[yy];
    ...
    case S2C_DELETE_RANGE:
    {
        ...
        if (r.y1 == r.y2)
        {
            ...
            if (yy < 0 || yy >= local_doc.num_lines)
                break;
            if (x1 < 0 || x2 > local_doc.line_lengths[yy])
                break;
        ...
        else
        {
            ...
            if (y1 < 0 || y2 >= local_doc.num_lines)
                break;
            if (x1 < 0 || x1 > local_doc.line_lengths[y1])
                break;
            if (x2 < 0 || x2 > local_doc.line_lengths[y2])
                break;
...
static bool attempt_reconnect(const char *server_ip)
{
    ...
    // Wait for user ID assignment before starting the receive thread.
    // This avoids a race where both this function and receive_handler read
    // from the same socket and one steals the handshake message.
    NetMessage id_msg;
    ...
    // Start receiver only after reconnect handshake is complete.
    pthread_t recv_thread;
    if (pthread_create(&recv_thread, NULL, receive_handler, &server_sock) < 0)
    {
        close(server_sock);
...
static void redraw(void)
{
    ...
    if (sel)
        selection_get_normalized(&sy, &sx, &ey, &ex);
    ...
    for (int y = 0; y < local_doc.num_lines; y++)
    {
        for (int x = 0; x < local_doc.line_lengths[y]; x++)
        {
            ...
            int attrs = 0;
            if (s_char.format & ATTR_BOLD)
                attrs |= A_BOLD;
            if (s_char.format & ATTR_UNDERLINE)
                attrs |= A_UNDERLINE;
            ...
            if (remote_pair)
                attron(COLOR_PAIR(remote_pair));
            attron(attrs);
            mvaddch(y, x, s_char.ch);
            attroff(attrs);
            if (remote_pair)
                attroff(COLOR_PAIR(remote_pair));
        }
    }
    ...
    if (current_format & ATTR_BOLD)
        strcat(format_str, "B ");
    if (current_format & ATTR_UNDERLINE)
        strcat(format_str, "U");
    ...
    // Connection status
    const char *conn_status = connected ? "Connected" : syncing ? "Syncing..."
                                                                : "OFFLINE";
...
static void selection_start_from_cursor(void)
{
    ...
    selection.ay = user_cursor.y;
    selection.ax = user_cursor.x;
    selection.hy = user_cursor.y;
    selection.hx = user_cursor.x;
}
static void selection_update_head_to_cursor(void)
{
    selection.hy = user_cursor.y;
